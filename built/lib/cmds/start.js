...
function start(options) {
    ...
    binaries[binaries_1.StandAlone.id].versionCustom = seleniumStandaloneVersion;
    ...
    seleniumProcess = utils_1.spawn('java', args, stdio);
    if (options[Opt.STARTED_SIGNIFIER].getString()) {
        signalWhenReady(options[Opt.STARTED_SIGNIFIER].getString(), options[Opt.SIGNAL_VIA_IPC].getBoolean(), outputDir, seleniumPort, options[Opt.VERSIONS_STANDALONE].getString(), downloadedBinaries[binaries_1.Appium.id] ? appiumPort : '', binaries[binaries_1.AndroidSDK.id], avdPort, androidActiveAVDs);
    }
...
function signalWhenReady(signal, viaIPC, outputDir, seleniumPort, seleniumStandaloneVersion, appiumPort, androidSDK, avdPort, avdNames) {
...
function shutdownEverything(seleniumStandaloneVersion, seleniumPort) {
    if (seleniumPort) {
        if (seleniumStandaloneVersion.startsWith('2')) {
            http.get('http://localhost:' + seleniumPort +
